...
export function validate(games) {
  ...
  for (const g of games) {
    ...
    // Where a game came from. Rules cluster by descent far more than by
    // geography — Uno's house rules are Crazy Eights house rules wearing a hat.
    if (g.lineage) {
      need(
        Array.isArray(g.lineage.from) && g.lineage.from.length,
        'lineage.from must list what this game descends from'
      );
      need(
        g.lineage.year === undefined || (Number.isInteger(g.lineage.year) && g.lineage.year > 1000),
        'lineage.year must be a four-digit year'
      );
      need(g.lineage.note, 'lineage needs a "note" saying what the descent actually means');
    }

    if (g.scores) {
      need(g.scores.typical, 'scores.typical — what an ordinary game looks like');
      need(g.scores.good, 'scores.good — what counts as playing well');
    }

    // Somewhere to actually play it. Only genuinely free places, and only ones
    // that have been checked — a dead or paywalled link is worse than none.
    for (const [i, p] of (g.play_online || []).entries()) {
      need(p.name, `play_online[${i}] needs a "name"`);
      need(/^https:\/\//.test(String(p.url || '')), `play_online[${i}].url must be an https URL`);
      need(typeof p.free === 'boolean', `play_online[${i}].free must be true or false`);
      need(p.note, `play_online[${i}] needs a "note" saying what you actually get`);
    }

    for (const [i, m] of (g.misplays || []).entries()) {
      need(m.move, `misplays[${i}] needs "move" — the honest mistake beginners make`);
      need(m.instead, `misplays[${i}] needs "instead" — what to do`);
    }
