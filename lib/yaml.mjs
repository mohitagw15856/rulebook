...
function parseMap(ctx, indent) {
  ...
  for (;;) {
    ...
    // A key is always a string. Running it through parseScalar turns `on:`
    // into the boolean true — the Norway problem, arriving via the key rather
    // than the value — which silently loses the field.
    const key = String(m[1]).trim().replace(/^["'](.*)["']$/, '$1');
